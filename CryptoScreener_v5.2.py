...
class App(tk.Tk):
    ...
    def create_dropdown_menu(self):
        ...
        display_names = ('Binance', 'Bybit', 'OKX', 'Bitget', 'BingX','Gate','MEXC')
    ...
    def run_program(self):
            ...
            if selected_exchange in ['binance', 'bybit']:
                ...
                def calculate_growth_percent(df):
                    ...
                    for i in range(1, 6):
                        ...
                        if (d['ma_30'] > d2['ma_30']  and d['ma_45'] > d2['ma_45'] and d['ma_60'] > d2['ma_60']):
                            
                            growth_percent[i-1, 0] = ((d['ma_30'] - d2['ma_30']) / d2['ma_30']) * 100
                            growth_percent[i-1, 1] = ((d['ma_45'] - d2['ma_45']) / d2['ma_45']) * 100
                            growth_percent[i-1, 2] = ((d['ma_60'] - d2['ma_60']) / d2['ma_60']) * 100
                            
                            growth_percent[i-1, 3] = ((d['ma_30'] - d['ma_45']) / d['ma_45']) * 100
                            growth_percent[i-1, 4] = ((d['ma_45'] - d['ma_60']) / d['ma_60']) * 100                            
                                  
                        else:
                            # 如果不符合MA遞增條件，則將增長百分比設置為 0
                            growth_percent[: , :] = 0
                            break
                    ...
                    # 篩選增長百分比遞增
                    # 檢查第一、第二、第三行是否遞增
                    check_column_0 = all(growth_percent[i, 0] > growth_percent[i + 1, 0] for i in range(4))
                    check_column_1 = all(growth_percent[i, 1] > growth_percent[i + 1, 1] for i in range(4))
                    
                    # 檢查第四列所有元素>第五列所有元素
                    check_column_4_gt_5 = all(growth_percent[i, 3] > growth_percent[i, 4] for i in range(5))
                    
                    # 如果任一條件不符合 返回 False
                    if not check_column_0 and check_column_1 and check_column_4_gt_5:
                        growth_percent[:, :] = 0
                ...
                def process_symbol(symbol, timeframe):
                    ...
                    if len(ohlcv) >= 66:  # 確保收集到足夠的數據
                        ...
                        return {symbol: strength_score}  # #回傳分數>0的標的
                ...
                for timeframe in timeframes:
                    ...
                    # 將 higher_base 中值低於 0 的元素刪除
                    higher_base = [item for item in higher_base if list(item.values())[0] > 0]
        ...
        elif selected_exchange in ['okx','bitget','bingx','gate','mexc']:
